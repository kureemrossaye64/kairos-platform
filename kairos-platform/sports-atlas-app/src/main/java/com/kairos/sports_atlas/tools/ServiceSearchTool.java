...
public class ServiceSearchTool {
    ...
    public String findServices(
        ...
        if (services.isEmpty()) {
            return "I'm sorry, I couldn't find any services, facilities, or courses matching your request.";
        }

        String formattedResults = services.stream()
                .map(service -> String.format(
                        "- %s (%s) in %s. Description: %s",
                        service.getName(),
                        service.getServiceType(), // e.g., "Public Sports Facility"
                        service.getLocation(),
                        service.getDescription()
                ))
                .limit(5) // Limit the number of results to keep the chat response concise

...
public class OpportunityTool {
    ...
    public String findTrainingOpportunities(
        ...
        SearchQuery.SearchQueryBuilder queryBuilder = SearchQuery.builder().textQuery(skill);

        // The AI might not provide a location, so this is optional.
        if (location != null && !location.isBlank()) {
            // We need GeocodingService here if we want to search by proximity
            // For simplicity in this tool, we'll just add it to the text query.
            // A more advanced version would use the geo capabilities.
        }
        
        SearchResult result = searchService.search( queryBuilder.build(), TrainingOpportunity.class);

        if (result.totalHits() == 0) {
            return "I'm sorry, I couldn't find any training opportunities matching '" + skill + "'.";
        }

        List<TrainingOpportunity> opportunities = opportunityRepository.findAllById(result.documentIds());
        
        String formattedResults = opportunities.stream()
                .map(opp -> String.format(
                        "- '%s' by %s (%s). Duration: %d weeks, Cost: Rs %.2f. Location: %s.",
                        opp.getTitle(),
                        opp.getProviderName(),
                        opp.getSkillCategory(),
                        opp.getDurationWeeks(),
                        opp.getCost(),
                        opp.getLocation()
                ))
